...
class Contoh2Activity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_contoh2)
        ...
        val videoPath =
            "android.resource://${packageName}/${R.raw.contoh2}" // Change "your_video_file" to your actual video file name
                                ...
                                .addOnSuccessListener { taskSnapshot ->
                                    fileReference.downloadUrl.addOnSuccessListener { uri ->
                                        ...
                                        databaseReference.child("users").child(username)
                                            .child("komentarcontoh2").setValue(komen)
                                        databaseReference.child("users").child(username)
                                            .child("filecontoh2").setValue(downloadUrl)
                                        databaseReference.child("users").child("list_pengguna")
                                            .child(username).child("komentarcontoh2")
                                            .setValue(komen)
                                        databaseReference.child("users").child("list_pengguna")
                                            .child(username).child("filecontoh2")
    ...
    private fun showDialog() {
        ...
        val nextDialog = Dialog(this)
        nextDialog.setContentView(R.layout.dialog_lanjut)
        nextDialog.setCancelable(false)
        nextDialog.window?.setBackgroundDrawableResource(android.R.color.transparent)

        val lanjut = nextDialog.findViewById<Button>(R.id.lanjut)
        lanjut.setOnClickListener {
            nextDialog.dismiss()
            this.finish()
            val intent = Intent(this, Contoh3Activity::class.java)
