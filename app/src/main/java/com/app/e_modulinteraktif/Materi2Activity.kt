...
class Materi2Activity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val videoPath = "android.resource://${packageName}/${R.raw.materi2}"
                                ...
                                .addOnSuccessListener { taskSnapshot ->
                                    fileReference.downloadUrl.addOnSuccessListener { uri ->
                                        ...
                                        databaseReference.child("users").child(username)
                                            .child("komentarmateri2").setValue(komen)
                                        databaseReference.child("users").child(username)
                                            .child("filemateri2").setValue(downloadUrl)
                                        databaseReference.child("users").child("list_pengguna")
                                            .child(username).child("komentarmateri2")
                                            .setValue(komen)
                                        databaseReference.child("users").child("list_pengguna")
                                            .child(username).child("filemateri2")
